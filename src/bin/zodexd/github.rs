...
use super::git_remote::{GitCredentialRequest, git_credential_request_repo};
...
pub(super) const DEFAULT_PUSH_GRANT_TTL_SECONDS: u64 = 30 * 60;
...
pub(super) fn normalize_github_repo(path: &str) -> Option<String> {
...
pub(super) fn parse_push_grant_ttl(raw: &str) -> Result<Duration> {
...
pub(super) fn load_matching_push_grant(
...
pub(super) fn parse_push_grants(raw: &str) -> Result<Vec<PushGrantRecord>> {
...
pub(super) async fn request_push_access(
    config: &Config,
    repo: &str,
...
pub(super) fn revoke_push_access(repo: &str, forget_local_auth: bool) -> Result<()> {
    let repo =
        normalize_github_repo(repo).ok_or_else(|| anyhow!("repo must be in owner/repo form"))?;
    let path = push_grant_path(&repo);
...
pub(super) fn list_push_grants() -> Result<()> {
    println!("grant-location: local");
...
#[cfg(test)]
pub(super) fn resolve_active_push_grant(repo: &str, grants_dir: &Path) -> Result<PushGrantRecord> {
