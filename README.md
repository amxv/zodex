...
Run it directly on a trusted Apple Silicon Mac or x86_64 Windows machine with **Zodex Local**, or use the same tool surface on a wake-on-demand remote Linux workspace with **Zodex Sprite**.
...
Local puts ChatGPT on your trusted macOS or Windows host through an **OpenAI Secure MCP Tunnel**. There is no public inbound port, proxy server, container, or separate remote machine between the model and the development environment you already use.

One Local runtime can serve multiple ChatGPT conversations at once. Zodex gives each conversation its own short Agent identity, keeps durable execution history, and lets you watch every command and file change in real time through the macOS **Liveboard** or the cross-platform terminal viewer.

- **Your real environment** — shell, Git, language toolchains, credentials, and files run as your logged-in user. macOS uses the captured login shell; Windows Local executes commands through Windows PowerShell with the captured user environment.
...
- **Host-native controls** — macOS includes an optional menu-bar app and browser Liveboard; Windows uses the terminal viewer and Windows Credential Manager.
...
Watch the Agents work. macOS opens Liveboard; Windows opens the terminal viewer:
...
| | Local | Sprite |
| --- | --- | --- |
| Host | Your Apple Silicon Mac or x86_64 Windows machine | Wake-on-demand remote Linux Sprite |
...
| Observability | macOS Liveboard + TUI; Windows TUI; durable history + HTTP/SSE API | Sprite service and operator diagnostics |
...
- [Local](https://zodex.ashray.xyz/docs/local) — trusted macOS/Windows execution, setup, observability, history, and daily use
